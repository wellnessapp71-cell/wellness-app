/**
 * POST /api/mental/content/progress — Update learning content progress
 * GET  /api/mental/content/progress — Get all content progress
 */
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    return errorResponse(500, "PROGRESS_SAVE_ERROR", "Unable to update content progress.", {
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    // Convert to a lookup map: moduleId -> progressPercent
    const progressMap: Record<string, number> = {};
    for (const p of progress) {
      progressMap[p.module] = p.progressPercent;
    }

    return ok({ progress: progressMap, entries: progress });
  } catch (error) {
    return errorResponse(500, "PROGRESS_FETCH_ERROR", "Unable to fetch content progress.", {
      message: error instanceof Error ? error.message : "Unknown error",
    });
  }
}
